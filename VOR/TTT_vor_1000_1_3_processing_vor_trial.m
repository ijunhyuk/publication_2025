function TTT_vor_1000_1_3_processing_vor_trial(data_fo, group_fo, mouse_fo, is_overwrite_vor_trial)
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
filter_window_pupil_move = 0.05; %sec. pupil cannot change quickly.
filter_window_pupil_size = 1; %sec. pupil cannot change quickly.
% filter_window_cr_movement = 0.5; %sec. cr cannot change quickly.
% filter_window_nan_fill = 0.015; %sec. to compensate the period where liklihood is less than threshold.(ex, when eye blink)
...
poor_DLC_percent = 90; %ex) 10 %. if the amount of poor detection is greater than this value, analysis is not proceed further.
saccade_ang_vel_thr = 40; %40 degree/sec. (Galliano, 2013, cell reports). 20220928. reduced to 20 because the fastest vestibular reflex
...
remove_around_saccade = 0.1; % sec. if velocity exceed saccade_ang_vel_thr degree/sec, then also remove before/after 30ms.
...
peak_gain_avg_time_window = 0.04; %20231205. sec. to get peak gain smoothed.
...
analysis_param.saccade_ang_vel_thr = saccade_ang_vel_thr;
