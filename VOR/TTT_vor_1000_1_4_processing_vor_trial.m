function TTT_vor_1000_1_4_processing_vor_trial(data_fo, mouse_fo, is_overwrite_vor_trial)
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
filter_window_pupil_move = 0.05; %sec. pupil size cannot change quickly.
filter_window_pupil_size = 1; %sec. pupil cannot change quickly.
filter_hz_multiply = 2; %filter for x,y position of pupil. stim_hz x n times
likelihood_thr = 0.80; %if likelihood is less than this, it will be marked as NaN.
poor_DLC_percent = 100; %ex) 10 %. if the amount of poor detection is greater than this value, analysis is not proceed further.
saccade_ang_vel_thr_default = 40; %40 degree/sec. (Galliano, 2013, cell reports). 20220928. reduced to 20 because the fastest vestibular reflex
saccade_ang_vel_thr_1hz = 60; %20231118, because of PC-TKO show more than 40, thr is changed to 
%is 20 degree/sec.(when 1hz 5degree VOR).
% remove_around_saccade = 0.05; % sec. if velocity exceed saccade_ang_vel_thr degree/sec, then also remove before/after 30ms.
remove_around_saccade = 0.1; % sec. if velocity exceed saccade_ang_vel_thr degree/sec, then also remove before/after remove_around_saccade sec.
% remove_around_saccade = 0.2; % sec. if velocity exceed saccade_ang_vel_thr degree/sec, then also remove before/after remove_around_saccade sec.
remove_around_bad_detection = 0.05; %sec. remove before/after 30ms.
...
peak_gain_avg_time_window = 0.06; %20231205. sec. to get peak gain smoothed.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
analysis_param.clr_pupil = clr_pupil;
analysis_param.v_fr = v_fr;
analysis_param.likelihood_thr = likelihood_thr;
analysis_param.poor_DLC_percent = poor_DLC_percent;
analysis_param.saccade_ang_vel_thr = saccade_ang_vel_thr_default;
analysis_param.saccade_ang_vel_thr_1hz = saccade_ang_vel_thr_1hz;
analysis_param.remove_around_saccade = remove_around_saccade;
analysis_param.peak_gain_avg_time_window = peak_gain_avg_time_window;
...
for fi=1:length(file_list) %fi = 24;
    clearvars 'vor_trial';
    vor_trial.analysis_param = analysis_param;
...
%     if contains(cur_file, '20230331_152435') % contains(cur_file, '20230331_155754')
%         'hi';
%     end
...
%         [filelist_st, rp_relative_path] = TTTH_search_all_files_ver2([data_fo filesep mouse_fo filesep exp_fo], 0, [radius_excel_prefix '*.xlsx'], [mouse_fo filesep exp_fo filesep]);
        [filelist_st, rp_relative_path] = TTTH_search_all_files_ver3([data_fo filesep mouse_fo filesep exp_fo], 0, [radius_excel_prefix '*.xlsx'], [mouse_fo filesep exp_fo filesep]);
...
%         [filelist_st, rp_relative_path] = TTTH_search_all_files_ver2([data_fo filesep mouse_fo filesep exp_fo], 0, [radius_excel_prefix '*.xlsx'], [mouse_fo filesep exp_fo filesep]);
        [filelist_st, rp_relative_path] = TTTH_search_all_files_ver3([data_fo filesep mouse_fo filesep exp_fo], 0, [radius_excel_prefix '*.xlsx'], [mouse_fo filesep exp_fo filesep]);
    elseif contains(trial_fo,'_Baseline')
%         is_vestibular_stim = 0;
%         is_visual_stim = 1; %0:VOR, 1:OKR, 2:VVOR
%         relative_fo = [mouse_fo filesep exp_fo filesep trial_fo];
%         mat_relative_path = [relative_fo filesep 'Baseline_trial_1.mat'];
%         filename = ['Baseline' filename_suffix];
%         video_relative_path = [relative_fo filesep filename '.mp4'];
%         csv_relative_path = [relative_fo filesep filename dlc_suffix '.csv'];
%         % filterd_csv_relative_path = [relative_fo filesep filename 'DLC_resnet_50_VOR_test_20220920Sep20shuffle1_100000_filtered.csv'];
%         [filelist_st, rp_relative_path] = TTTH_search_all_files_ver2([data_fo filesep mouse_fo filesep exp_fo], 0, [radius_excel_prefix '*.xlsx'], [mouse_fo filesep exp_fo filesep]);
    ...
    if exp_hz == 1 && (strcmp(exp_type,'VOR') || strcmp(exp_type,'VVOR')) % specifically for VOR 1Hz.
        saccade_ang_vel_thr = saccade_ang_vel_thr_1hz;
    else
        saccade_ang_vel_thr = saccade_ang_vel_thr_default;
    end
    ...
    % make it nan whereever likelihood is below threshold.
    pad_count = round(remove_around_bad_detection*v_fr);

    pt_str = 'cr';
    pt = dlc_st.(pt_str);
    bad_detection_idx = (pt.likelihood < likelihood_thr);
    bad_detection_idx = TTT_vor_1000_1_3_1_sub_idx_padding(bad_detection_idx, pad_count);
    pt.x(bad_detection_idx) = nan;
    pt.y(bad_detection_idx) = nan;
    dlc_st.(pt_str) = pt;
...
%     dlc_st = TTT_vor_1000_10_1_filter_dlc(dlc_st, likelihood_thr); % filter sudden mis-detections.
    ...
    %plots
    fig1 = figure('color',[1 1 1], 'position', fig_size,'Visible', fig_visible);
    ...
    nan_idx = isnan(pupil_move);
    
%     pupil_move_filtered = fillmissing(pupil_move_filtered,'spline');
% %     pupil_move_filtered = smooth(pupil_move_filtered,round(filter_window_nan_fill*v_fr));        
%     %lowpass filter with proper filter hertz(filter_hz_ang_vel)
%     if ~isempty(find(~isnan(pupil_move_filtered)))
% %         pupil_move_filtered = lowpass(pupil_move_filtered,filter_hz_ang_vel,v_fr);
%         [pupil_move_filtered] = TTTH_a24_2_ver1_low_pass_filter(pupil_move_filtered, v_fr, filter_hz_ang_vel, 2);
%     end
    
    %20230415, Remove 1 pixel jittering in DLC detection. Apply a minimum filtering. 2 times.
%     pupil_move_filtered = medfilt1(pupil_move_filtered,round(filter_window_pupil_move*v_fr),'omitnan','truncate');
    pupil_move_filtered = smooth(pupil_move_filtered,round(filter_window_pupil_move*v_fr));
    pupil_move_filtered = smooth(pupil_move_filtered,round(filter_window_pupil_move*v_fr/2));
    
    pupil_move_filtered(nan_idx) = NaN;
...
%     p_diff_ang = asind(p_diff_px./radius_pupil(2:end));
    ...
    TTT_vor_1000_11_2_hist_eye_velocity(cur_ax, p_ang_vel);
    
    
    p_ang_vel_filterd = p_ang_vel;
    %remove couple of values (r_sac) around speed over than +-40 degree/sec.
    ...
    sac_mask = TTT_vor_1000_1_3_1_sub_idx_padding(sac_mask, r_sac);
    p_ang_vel_filterd(sac_mask) = nan;
    
    if ~isempty(find(~isnan(p_ang_vel_filterd)))
        % filter the velocity depends on expeirmental Hz.
        nan_idx = isnan(p_ang_vel_filterd);
    ...
    % consider the delay of visual presentation by the beam projector.
    delay_vis = vor_s.vis_command.sync_dely; % msec.
    delay_vis_nidx = round(cmd_fr*delay_vis/1000);
    vis_pos_shift = [zeros(delay_vis_nidx,1); vis_pos(1:end-delay_vis_nidx)];
    vis_pos = vis_pos_shift;
...
%     exp_type = vor_s.exp_type;
    ...
    if is_visual_stim==1
        ...
        vis_vel = lowpass(vis_vel,filter_hz_ang_vel,v_fr,'Steepness',0.99);     % to prevent the values spiking.
    else
    ...
    % vor_s.table_rotation_command.t_run_extra property
...
%     (prefix_time + run_time + suffix_time)*new_cmd_fr;
    ...
    sm_ang_vel = smooth(p_ang_vel_filterd,(1/stim_freq)/2*new_cmd_fr);
    sm_ang_vel = p_ang_vel_filterd;
    ...
    legend_str{end+1} = 'Gain';
    plot(xx_peak_pos, peak_gains,'o','color',[0 0.5 0]);

    box off; hold on;
    legend_str{end+1} = ['Median Gain= ' num2str(gain_median)];
    plot([xx(1),xx(end)], [gain_median gain_median],'--','color',[0 0.5 0]);
    %draw run_window
    legend_str{end+1} = '';
    plot([xx_run(1), xx_run(1)], [0, 1.5], '--k');
    legend_str{end+1} = '';
    plot([xx_run(end), xx_run(end)], [0, 1.5], '--k');
    %draw peaks
    for i=1:length(xx_peak_pos)
        legend_str{end+1} = '';
        x = xx_peak_pos(i);
        plot([x, x], [0, 1.5], '--', 'color',[0.5,0.5,0.5]);
    end

    xlabel('(sec)'); ylabel(['Gain (eye vel/' cur_vel_str ' vel)']); %ylim([-1.5 1.5]);
    yyaxis right;
    legend_str{end+1} = [cur_vel_str ' vel'];
    plot(xx, cur_vel,'color',[0.5 0.5 1]);
    legend_str{end+1} = ['pupil vel.'];
    plot(xx, p_ang_vel_filterd,'-','color',[1 0 0]);
    box off; hold on; ylim(['auto']);
%     plot(ni_xx(run_idx), sm_run_stim_vel,'-','color',[0.5 0.5 0.5]);
%     plot(ni_xx(run_idx), sm_run_ang_vel,'-','color',[1 0 1]);
%     legend({'Gain',['Median Gain= ' num2str(gain_median)],[cur_vel_str ' vel'],['pupil vel.'],['phase ' cur_vel_str],['phase pupil vel']},'interpreter','none','Location','southeast');
    % legend({'Gain',['Median Gain= ' num2str(gain_median)],[],[],[cur_vel_str ' vel'],['pupil vel.']},'interpreter','none','Location','southeast');
    legend(legend_str,'interpreter','none','Location','southeast');

    title([title_str ', (n_peak_gains = ' num2str(length(find(~isnan(peak_gains)))) '/' num2str(size(peak_gains,1)) ')']);
