function [eye_p_cs_us, median_cs_us, eye_p_cs_only, median_cs_only, is_squint_cs_only, is_squint_cs_us] = ...
    TTT_eyeb100_7_2_normalize_eye_blink(eyeB_st, is_filter_squint, squint_thr_px)
...
avg_base_res = nanmedian(eye_h_cs_us(base_s:base_e, :),1)';
is_squint_cs_us = abs(avg_base_res) < squint_thr_px;
avg_base_res = nanmedian(eye_h_cs_only(base_s:base_e, :),1)';
is_squint_cs_only = abs(avg_base_res) < squint_thr_px;

if is_filter_squint
    eye_h_cs_us(:,is_squint_cs_us) = nan;
    eye_h_cs_only(:,is_squint_cs_only) = nan;
end
