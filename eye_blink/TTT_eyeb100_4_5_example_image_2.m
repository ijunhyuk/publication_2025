function [] = TTT_eyeb100_4_5_example_image_2()
%%
% 20221213. Joon.
warning off;
...
mouse_ids_ctr = 'm1002';
mouse_ids_tko = 'm1001,m1004';

data_search_fo_arr = {};
data_search_fo_arr{end+1} = 'M:\Dropbox (HMS)\BigData_HMS\EyeBlink\EyeBlink_Data';
...
for v=1:length(vari)
    ...
    for t=1:length(trial_no_arr)
        ...
        if is_only_show_one_image
            save_img_path = [save_video_fo filesep file_name_prefix '_' fi '_trial_' num2str(trial_no) name_str '.jpg'];
            if ~exist(save_video_fo)
                mkdir(save_video_fo);
            end
