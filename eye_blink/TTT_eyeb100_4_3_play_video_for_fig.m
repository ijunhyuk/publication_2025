...
% cur_vari.mat_path = [cur_vari.video_fo '\Sess_Info_20230720_120903.mat'];
% % cur_vari.trial_no_arr = [24];   
...
% cur_vari.save_video_fo = [cur_vari.video_fo '\analysis'];
% vari{end+1} = cur_vari;
...
% cur_vari.video_path = [cur_vari.video_fo '\B4_20230716_102111.mp4']; box_no = 4;
% % cur_vari.trial_no_arr = [91];   
% cur_vari.trial_no_arr = [    
%     10
%     16
...
for v=1:length(vari)
    ...
    if is_only_show_one_image
        save_img_path = [save_video_fo filesep fi '_trial_' num2str(trial_no) '.jpg'];
        if ~exist(save_video_fo)
            mkdir(save_video_fo);
        end
