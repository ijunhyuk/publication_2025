classdef Violin_JL_with_mean_sem < handle
    ...
    methods
        function obj = Violin_JL_with_mean_sem(data, pos, varargin)
...
%                 obj.MedianPlot.MarkerEdgeColor = args.EdgeColor;
                obj.MedianPlot.MarkerEdgeColor = args.BoxColor;
                return
            end
...
%             quartiles = quantile(data, [0.25, 0.5, 0.75]);    
            %replace quartiles to [mean-sem sem mean+sem];
            mean_val = mean(data,1);
            sem_val = std(data)/sqrt(size(data,1));
            quartiles = [mean_val-sem_val mean_val mean_val+sem_val];
...
%             obj.ScatterPlot = ...
%                 scatter(pos + jitter.*jitterstrength, data, 'o');
            ...
            %use lowhisker as mean-sem, hiwhisker as mean+sem.
...
%                          [], [1 1 1], 'filled', 'v');
            ...
            obj.MeanPlot = plot(pos+[-1,1].*meanDensityWidth*3/4, ...
                                [meanValue, meanValue],'k','linewidth',1);
            obj.MeanPlot.LineWidth = 1;
                 
            SemPlot_high = plot(pos+[-1,1].*meanDensityWidth*3/4/2, [quartiles(1) quartiles(1)],'k','linewidth',1);
            SemPlot_vertical = plot([pos pos], [quartiles(1) quartiles(3)],'k','linewidth',1);
            SemPlot_low = plot(pos+[-1,1].*meanDensityWidth*3/4/2, [quartiles(3) quartiles(3)],'k','linewidth',1);
...
%             obj.ScatterPlot.MarkerFaceColor = color+(1-color)*0.4;            
            obj.ScatterPlot.MarkerFaceColor = color;            
            obj.ScatterPlot.MarkerEdgeColor = color+(1-color)*0.4;
%             obj.ScatterPlot.MarkerEdgeColor = color;
