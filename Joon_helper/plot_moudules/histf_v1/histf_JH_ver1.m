function [n,xout,h] = histf_JH_ver1(Y,varargin)
...
%  [n,xout,h] = histf(...)
%  
%     histf(vels,hist_edge,'facecolor',rgb,'FaceAlpha',hist_face_alpha,...
%         'BarWidth',0.9,'edgecolor','none');

%         histf_JH_ver1(isi_msec,hist_edge_msec,'facecolor',rgb_total,'FaceAlpha',min([1,hist_face_alpha*2]),...
...
tmp = strcmpi(varargin,'area_normalize'); 
if any(tmp)
    area_normalize = varargin{find(tmp)+1};
    assert(isscalar(area_normalize)==1,'BarWidth must be a scalar value between 0 and 1.') 
    tmp(find(tmp)+1)=1; 
    varargin = varargin(~tmp); 
end    
...
set(h,'FaceAlpha',FaceAlpha,'EdgeAlpha',EdgeAlpha);
